...
export default function PatientsTable({ patients }: Props) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-2 sm:p-6 bg-gray-50 min-h-screen">
      <div className="hidden sm:grid grid-cols-5 px-6 py-3 bg-white border-b text-sm font-medium text-gray-600">
      ...
      <div className="space-y-3 sm:space-y-2 mt-2">
        {patients.map(p => (
          <div
            key={p.id}
            className="flex flex-col sm:grid sm:grid-cols-5 items-start sm:items-center px-4 py-4 sm:px-6 bg-white rounded-xl sm:rounded-lg shadow-sm hover:shadow-md transition-shadow relative gap-3 sm:gap-0"
          >
