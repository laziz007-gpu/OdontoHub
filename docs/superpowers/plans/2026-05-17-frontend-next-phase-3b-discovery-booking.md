...
- `gosmile:booking_doctor` → JSON of the chosen doctor object. **Written** by 3a `SuggestedDoctors`, and (3b) by `Doctors/DoctorsList`, `Doctors/DoctorCard`, `SearchResultsPage`, `DoctorProfilePreview`. **Read + cleared** by `/booking`.
- `gosmile:doctors_specialty` → specialty string. **Written** by 3a `ServicesGrid`. **Read + cleared** by `/doctors` (`DoctorsList`).
